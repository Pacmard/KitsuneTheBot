...
module.exports = {
  name: 'changejoin',
  description: 'if you want to change location of join logs',
    ...
    if (perms.has('ADMINISTRATOR')) {
      ...
      if (isEnabled) {
        if (isEnabled.logschannel !== msg.channel.id) {
          const removingCmdsVars = msg.content.replace('k!changejoin', '');
      ...
      return msg.reply('Logging is not enabled in this server! If you want to enable it, use `k!enablejoin` command!');
    }
    return msg.reply('You don\'t have admin permissions to change join logs channel!');
  },
};
